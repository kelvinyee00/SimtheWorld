...
### 24) Iteration 11 Roadmap — Interactive Dashboard & Real-time Tuning (P11)

#### 24.1 Dashboard Sink Blocks
- Introduce **Gauge** and **Lamp** blocks for direct canvas visualization.
- Gauge: Renders a numeric needle or bar indicator directly within the node's custom view.
- Lamp: Renders a multi-color status indicator based on boolean or integer input (e.g., 0=gray, 1=green, 2=red).

#### 24.2 Interactive Source Blocks
- Introduce **Knob** and **Slider** blocks for manual input.
- These blocks allow users to change values in the model during an active simulation run without opening the inspector.
- Changes must propagate deterministically to the next tick evaluation.

#### 24.3 Real-time Wall-clock Synchronization
- Add a "Real-time" toggle to the simulation toolbar.
- When active, the scheduler pauses execution between ticks to match the `stepTimeMs` against the actual system clock.
- Critical for HMI testing and visual monitoring.

#### 24.4 Live Edge Probing
- Implement a hover-based visualization for connection lines.
- Hovering over an edge during or after a run should show the last emitted signal value in a non-obstructive tooltip.
- Leverages `nodeOutputs` state for O(1) value lookup.

#### 24.5 P11 Verification
- Full regression sweep across P0-P11.
- Real-time engine pacing accuracy verification.
- Mobile touch-interaction check for Knob/Slider components.
